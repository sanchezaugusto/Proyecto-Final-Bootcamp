...
export default function Cart({user}){
    ...
    const handleOrders = async () => {
        ...
        const orderData = {
            user_id: user.userId, // Reemplaza esto con el ID del usuario logueado
            products: items,
    ...
    const handlePayment = async () => {
        ...
        try {
          console.log("hola", `${process.env.NEXT_PUBLIC_API_HOST}/mails/sendMail`)
          const mailRequest = await fetch(`${process.env.NEXT_PUBLIC_API_HOST}/mails/sendMail`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({items, mail: user.email}),
          });
